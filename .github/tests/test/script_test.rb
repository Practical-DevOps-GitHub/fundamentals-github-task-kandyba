...
class ScriptTest < Test::Unit::TestCase
  ...
  def test_approve_from_user
    user_name = 'softservedata'
    rules = @obj.rules_required_pull_request_reviews('main')
    classic_require_code_owner_review = rules && rules["require_code_owner_reviews"]
    pull_request_rulesets_rules = @obj.get_branch_ruleset('main')
    rulesets_require_code_owner_review = pull_request_rulesets_rules&.find { |rule| rule['type'] == 'pull_request' }&.dig('parameters', 'require_code_owner_review')
    assert(classic_require_code_owner_review || rulesets_require_code_owner_review,
      "We should not allow merge to main branch without approve from #{user_name}")
  end
